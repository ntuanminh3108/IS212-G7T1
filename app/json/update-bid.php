...
if (empty($errors) && isset($_REQUEST['r'])) {
    ...
    if (!isset($request->amount)) { // not $request['amount'] because $request is a PHP Object.
...
/*  if no errors with the request, update the bid.
    Returns the status of the delete-bid, and appropriate error messages (if any).
*/
else {
    $result = updateBid($request->userid,$request->amount,$request->course,$request->section);
}
